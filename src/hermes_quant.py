...
RISK_MAX_PER_TRADE = 0.005     # 0.5%
RISK_DAILY_MAX = 0.01          # 1%
RISK_WEEKLY_MAX = 0.03         # 3%
            ...
            if tool_name in self.tools:
                try:
                    ...
                    # ── L2: Analysis Layer ──
                        ...
                        result = tool.analyze(args[0]) if len(args) >= 1 else tool.analyze('XAUUSD')
                        ...
                        result = tool.detect_regime(args[0]) if len(args) >= 1 else tool.detect_regime('XAUUSD')
                    # ── L3: Decision Layer ──
                    ...
                    # ── L4: Execution Layer ──
                        ...
                        result = tool.status() if hasattr(tool, 'status') else "AutoSwitch active"
                    # ── L5: Learning Layer ──
                    elif tool_name == 'audit':
                        result = tool.get_recent() if hasattr(tool, 'get_recent') else tool.status() if hasattr(tool, 'status') else str(len(tool.entries)) + " audit entries" if hasattr(tool, 'entries') else "Audit logger active"
